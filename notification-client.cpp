...
inline bool NotificationClient::valid() const
{
	return !!bs && !bs->destroying;
}

CefRefPtr<CefLoadHandler> NotificationClient::GetLoadHandler()
{
	return this;
}

CefRefPtr<CefRenderHandler> NotificationClient::GetRenderHandler()
{
	return this;
}

CefRefPtr<CefDisplayHandler> NotificationClient::GetDisplayHandler()
{
	return this;
}

CefRefPtr<CefLifeSpanHandler> NotificationClient::GetLifeSpanHandler()
{
	return this;
}

CefRefPtr<CefContextMenuHandler> NotificationClient::GetContextMenuHandler()
{
	return this;
}

CefRefPtr<CefAudioHandler> NotificationClient::GetAudioHandler()
{
	return reroute_audio ? this : nullptr;
}

#if CHROME_VERSION_BUILD >= 4638
CefRefPtr<CefRequestHandler> NotificationClient::GetRequestHandler()
{
	return this;
}
	...
	case ControlLevel::Advanced:
		if (name == "startReplayBuffer") {
			...
			if (!source) {
				...
				blog(LOG_WARNING, "Notification source '%s' tried to switch to '%s' which isn't a scene",
				     obs_source_get_name(bs->source), scene_name.c_str());
			} else {
			...
			if (transition)
				...
				blog(LOG_WARNING,
				     "Notification source '%s' tried to change the current transition to '%s' which doesn't exist",
				     ...
				     const CefString &source, int line)
{
	...
	blog(errorLevel, "[spt-notification: '%s'] %s: %s (%s:%d)", sourceName, code, message.ToString().c_str(),

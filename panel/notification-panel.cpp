...
struct QCefCookieManagerInternal : QCefCookieManager {
	...
	QCefCookieManagerInternal(const std::string &storage_path, bool persist_session_cookies)
	{
		if (os_event_try(cef_started_event) != 0)
			throw "Notification thread not initialized";
...
void QCefWidgetInternal::closeNotification()
{
	...
	if (!!browser) {
		...
		void *view = (id)cefNotification->GetHost()->GetWindowHandle();
		if (*((bool *)view))
...
void QCefWidgetInternal::Resize()
{
	...
	bool success = QueueCEFTask([this, size]() {
		...
		CefWindowHandle handle = cefNotification->GetHost()->GetWindowHandle();
...
void QCefWidgetInternal::setURL(const std::string &url_)
{
	url = url_;
	if (cefNotification) {
		cefNotification->GetMainFrame()->LoadURL(url);
	}
}
...
{
	if (cefNotification)
		cefNotification->ReloadIgnoreCache();
}
...
void QCefWidgetInternal::executeJavaScript(const std::string &script_)
{
	...
	CefRefPtr<CefFrame> frame = cefNotification->GetMainFrame();
